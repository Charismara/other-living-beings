...
public class BounceHelper implements Consumer<LivingEvent.LivingUpdateEvent> {
    private static final IdentityHashMap<Entity, BounceHelper> bouncingEntities = new IdentityHashMap<>();
    ...
    public BounceHelper(LivingEntity entityLiving, double bounce) {
    ...
    @Override
    public void accept(final LivingEvent.LivingUpdateEvent event) {
    ...
    public static void addBounceHandler(LivingEntity entity, double bounce) {
        ...
        BounceHelper handler = bouncingEntities.get(entity);
        if (handler == null) {
            // wasn't bouncing yet, register it
            MinecraftForge.EVENT_BUS.addListener(new BounceHelper(entity, bounce));
        } else if (bounce != 0) {
